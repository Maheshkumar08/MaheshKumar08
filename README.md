<!--Header Name-->
<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=40&duration=3000&pause=1000&color=38BDF8&center=true&vCenter=true&width=600&lines=Hi%2C+I'm+Mahesh+Vyas+%F0%9F%91%8B;Full+Stack+Developer;" alt="Typing SVG" />
</p>


<br />
<div>
  <img align="right" width="40%" src="https://user-images.githubusercontent.com/74038190/235224431-e8c8c12e-6826-47f1-89fb-2ddad83b3abf.gif">
</div>
<!--Start Intro-->
<p align="left">
🚀 Full Stack & Mobile App Developer skilled in React.js, Node.js, Express.js, MongoDB, React Native, JavaScript, Redux Toolkit, REST APIs, and Firebase. Experienced in developing modern web applications and mobile solutions with a focus on performance, scalability, and clean code. Always eager to learn, build innovative products, and solve real-world problems through technology.
</p>

- 🚀 Full Stack & Mobile App Developer specializing in MERN Stack and React Native
- ✨ Passionate about building clean UI, scalable applications, and great user experiences
- 🌱 Currently improving my skills in Next.js, TypeScript, and System Design
- 💻 Experienced in React.js, Node.js, Express.js, MongoDB, React Native, Redux Toolkit, Firebase, and REST APIs
- 🔥 Skilled in authentication, payment integrations, admin dashboards, and real-time applications
- 🚀 Focused on performance optimization, reusable component architecture, and clean code practices
- 📱 Love developing both web and mobile applications that solve real-world problems
- ❤️ Always learning new technologies and building personal projects

- 🌐 Visit my **Portfolio**: [Portfolio](https://your-portfolio-link.com)
- 📫 Reach me at: **your-email@gmail.com**
- 💼 LinkedIn: **https://linkedin.com/in/your-profile**
<!--End Intro-->

<!--Profile Count Badge-->
<p align="left">
  <img src="https://komarev.com/ghpvc/?username=deepakkumar410&label=Profile%20views&color=770677&style=for-the-badge&logo=star" />
</p>

---

<!--Tech Stack-->
<h2 align="center">Tᴇᴄʜ sᴛᴀᴄᴋ & Lᴀᴛᴇsᴛ Wᴏʀᴋ</h2>

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="./Skills_Animation_Dark.gif">
  <source media="(prefers-color-scheme: light)" srcset="./Skills_Animation_White.gif">
  <img align="left" alt="Skills Animation" src="./Skills_Animation_Dark.gif">
</picture>
<br />

<h3 align="left">Current Learning</h3>
<ul align="left">
  <li>🚀 Advanced React.js, Next.js & frontend performance optimization</li>
  <li>⚙️ Backend development with Node.js, Express.js & REST APIs</li>
  <li>🗄️ Database design and management using MongoDB & MySQL</li>
  <li>📱 Cross-platform mobile app development with React Native</li>
  <li>🔐 Authentication, JWT, Firebase & role-based access control</li>
  <li>🔄 State management using Redux Toolkit & RTK Query</li>
  <li>🌐 Real-time applications using Socket.io</li>
  <li>☁️ Deployment using Git, GitHub, VPS & cloud platforms</li>
</ul>

<h3 align="left">Highlighted Projects</h3>
<ul align="left">
  <li><a href="https://github.com/USERNAME/ecommerce-app">🛒 E-Commerce App (MERN Stack)</a></li>
  <li><a href="https://github.com/USERNAME/job-portal">💼 Job Portal (React + Node.js + MongoDB)</a></li>
  <li><a href="https://github.com/USERNAME/chat-app">💬 Real-Time Chat App (Socket.io)</a></li>
  <li><a href="https://github.com/USERNAME/react-native-app">📱 React Native Mobile App</a></li>
</ul>

<br /><br /><br /><br />

---

<!--Trophies-->
<!-- <h2 align="center">🏆 Gɪᴛʜᴜʙ Tʀᴏᴘʜɪᴇs 🏆</h2>
<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=deepakkumar410&row=2&column=6&theme=monokai&no-bg=true&margin-w=20&margin-h=20" />
</p> -->

---

<!--Github Stats-->
<!-- <h2 align="center">📊 Gɪᴛʜᴜʙ Sᴛᴀᴛs 📊</h2>

<table width="100%">
  <tr>
    <td width="50%">
      <h3 align="center">Gɪᴛʜᴜʙ Sᴛᴀᴛs</h3>
      <img src="https://github-readme-stats.vercel.app/api?username=deepakkumar410&show_icons=true&theme=nightowl" />
    </td>
    <td width="50%">
      <h3 align="center">Sᴛʀᴇᴀᴋ Sᴛᴀᴛs</h3>
      <img src="https://github-readme-streak-stats.herokuapp.com/?user=deepakkumar410&theme=nightowl" />
    </td>
  </tr>
</table> -->

---

<!--Contribution Graph-->
<!-- <h2 align="center">📈 Cᴏɴᴛʀɪʙᴜᴛɪᴏɴ Gʀᴀᴘʜ 📈</h2>
<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=deepakkumar410&bg_color=220a28&color=ffffff&line=c56a90&point=ffeb95" />
</p> -->



<!--Contact Section-->
<h2 align="center">🤝 Connect With Me 🤝</h2>

<p align="center">
  <a href="mailto:maheshk00328@gmail.com">
    <img src="https://cdn-icons-png.flaticon.com/512/732/732200.png" width="45" />
  </a>

  <a href="https://github.com/Maheshkumar08">
    <img src="https://cdn-icons-png.flaticon.com/512/733/733553.png" width="45" />
  </a>

  <a href="https://www.linkedin.com/in/mahesh-kumar-vyas-286980319/">
    <img src="https://cdn-icons-png.flaticon.com/512/174/174857.png" width="45" />
  </a>
</p>

<br/>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=65&section=footer"/>
</p>
